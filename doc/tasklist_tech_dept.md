...
| 1 | Критичный рефакторинг | ⏳ Ожидание | 0/4 | Тесты проходят | - |
| 2 | Типизация | ⏳ Ожидание | 0/3 | `mypy` без ошибок | - |
| 3 | Расширение тестов | ⏳ Ожидание | 0/4 | Coverage 80%+ | - |
| 4 | Финальная полировка | ⏳ Ожидание | 0/3 | Все проверки зеленые | - |

**Легенда:** ⏳ Ожидание | 🔄 В работе | ✅ Завершено | ❌ Заблокировано

**Прогресс:** 4/18 задач (22%)
...
- [ ] Убрать глобальную загрузку config в `handlers/messages.py:15`
- [ ] Реализовать singleton/кэширование для OpenAI клиента в `services/llm.py`
- [ ] Создать `constants.py` с enum `MessageRole` вместо magic strings
- [ ] Обновить все места использования с проверкой работоспособности
...
- [ ] Добавить type hints во все публичные функции (если отсутствуют)
- [ ] Добавить аннотации для переменных где неоднозначно
- [ ] Исправить все ошибки mypy
- [ ] Создать типы для словарей (TypedDict для context)
...
- [ ] Финальная проверка соответствия conventions.mdc и vision.md
